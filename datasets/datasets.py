...
class Sines(Dataset):
    ...
    def _generate_sines(self):
        ...
        '''
        generate a n_series*datapoints results
        '''
...
class Financial_index(Dataset):

    def __init__(self, datapath: str = '000001.SS.csv', datapoints: int = 252):
        """
        Pytorch Dataset to produce real financial series.

        """
        self.datapath = datapath
        self.datapoints = datapoints
        self.dataset = self._generate_fins()
    ...
    def _generate_fins(self):
        ...
        data = data[data['Close'].isna()==False]
        samples = [[]]
        data['Close'].rolling(252).apply((lambda x: samples.append(x.tolist()) or 0), raw=False)
        samples = samples[1:]
...
# arma = ARMA(0.7, 0.2)
# fake_data = arma._generate_ARMA()

# import matplotlib.pyplot as plt 

# plt.plot(range(len(fake_data[0])), fake_data[1])